...
export const useSolanaAction = (): SolanaAction => {
  ...
  const sendActionTransaction = useCallback(async (memo: string): Promise<SolanaActionResult> => {
    if (!publicKey) {
    ...
    try {
      ...
      if (balance < 5000) { // Need at least 0.000005 SOL for transaction
        const error = new Error("Insufficient SOL balance. Please request an airdrop first.");
        console.error(error);
        return { signature: null, error };
      }
  ...
  return { sendTransaction: sendActionTransaction, requestAirdrop, getBalance, isSending };
};